...
3. 所以继续改进，不由 mon 来生成唯一的数字标识，而是通过哈希算法来得到不同的输出
   ...
   再针对每个 PG 的处理流程，还需要考虑：
   
   - 内存消耗：哈希结果需要作为对象标识的一部分常驻内存
   - 比较效率：引入这个数值的初衷就是为了对对象排序进行加速
   
   所以，最终决定采用 32 位哈希输出
   
   

确定了哈希输出的长度后，也需要确认哪些对象特征值适合充当哈希输入

- 最直接的方法，就是直接使用对象的全部特征值，但这也是最差的方法，因为快照、纠删码数据回滚等操作其实并不是日常操作，所以与之相关的快照标识、分片标识、版本号等在大部分情况下，也都是无效值
- 前文说到，某个 PG 中的对象只能归属于同一个存储池，所以计算哈希时，存储池标识也是无效输入

综上，最终认为使用命名空间加上对象名作为哈希输入是合适的



# 4. stable_mod 与客户端寻址

Ceph 通过 C/S 模式实现外部程序与 RADOS 集群的交互，那么应用程序通过客户端访问集群时，就需要：

1. 首先，由客户端生成一个字符串形式的对象名，然后基于这个对象名计算得出一个 32 位哈希值【如前所述，这个输出可能还要加上命名空间等特征值】
2. 其次，对这个 32 位哈希输出再做一些运算，得到对象对应的 PG 在存储池内部的编号
3. 最后，PG 编号加上对象本身已经记录的其归属的存储池的唯一标识就可以找到最终负载承载该对象的 PG

```
假设现有一个对象，其归属于表示为 1 的 pool，经过计算后的 32 位哈希值为 0x4979FA12 0x4979FA12，并且该 pool 的 pg_num 为 256
那么可有：
0x4979FA12 mod 256 =18
就可以知道，该对象由 1 pool 中的 18 PG 承载，所以完整的 PGID 为 1.18
```

不过，一般而言，由于 ceph 的一些特性，其实并不会使用全部的 32 位哈希值，因此会出现不同的对象被映射到同一个 PG 的现象
...
这里就可以看到，其实只使用了这个哈希值的后八位
```

如果 pg_num 可以写成 2 的 n 次方形式，则每个对象进行 PG 映射时，其实这个 32 位哈希值里只有低 n 位是有意义的

```
例如上面的例子中，256 = 2^8
那么对象执行 PG 映射时，也只需要使用后八位
```

所以可以得出，这种情况下归属于同一个 pg 的对象，它们的 32 位哈希值中的低 n 位都是有意义的，为此，引入了 pg_num 掩码的概念，pg_num 的掩码就是 （2^n）-1

不过，上述的例子里 PG_num 是刚好为 2 的幂等数，实际情况却并非总是如此，所以还需要考虑 pg_num 不为 2 的幂等的情况，因此要对简单的 mod 操作加以改进，以保证针对不同输入，当计算结果相等时，维持“输入具有尽可能多的、相同的低位”

1. 一种方案是使用前文说到的掩码来代替取模操作，
